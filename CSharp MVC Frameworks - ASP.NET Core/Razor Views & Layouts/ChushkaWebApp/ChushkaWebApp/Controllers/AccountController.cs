namespace ChushkaWebApp.Controllers
{
    ...
    using ChushkaWebApp.Models.AccountViewModels;
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (this.ModelState.IsValid)
            {
                var usersCount = this._userManager.Users.Count();
                ...
                if (result.Succeeded)
                {
                    if (usersCount == 0)
                    {
                        bool hasAdminRole = await this._roleManager.RoleExistsAsync("Admin");
                        if (!hasAdminRole)
                        {
                            var role = new IdentityRole { Name = "Admin" };
                            await this._roleManager.CreateAsync(role);
                        }
                        result = await this._userManager.AddToRoleAsync(user, "Admin");
                    }
                    else
                    {
                        bool hasUserRole = await this._roleManager.RoleExistsAsync("User");
                        if (!hasUserRole)
                        {
